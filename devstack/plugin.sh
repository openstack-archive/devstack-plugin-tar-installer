...
PLUGIN_DIR=$DEST/devstack-plugin-tar-installer/devstack


function install_tar_installer_plugin {

    if [[ -n "$QEMU_VERSION" ]]; then
        source $PLUGIN_DIR/build_qemu.sh "$QEMU_VERSION"
    fi
    if [[ -n "$LIBVIRT_VERSION" ]]; then
        source $PLUGIN_DIR/build_libvirt.sh "$LIBVIRT_VERSION"
    fi
}
...
function remove_tar_installer_plugin {
    ...
    if [[ -n "$QEMU_VERSION" ]]; then
        source $PLUGIN_DIR/build_qemu.sh remove
    fi
    if [[ -n "$LIBVIRT_VERSION" ]]; then
        source $PLUGIN_DIR/build_libvirt.sh remove
    fi
...
if is_service_enabled devstack-plugin-tar-installer; then
